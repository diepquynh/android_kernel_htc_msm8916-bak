...
static void qpnp_bms_write_emmc(struct work_struct *work)
{
	...
	if (the_chip->dt.store_batt_data_soc_thre > 0
		...
		rc = emmc_misc_write(store_emmc.store_currtime_ms/1000, BMS_STORE_CURRTIME_OFFSET);
		if (!rc)
			write_fail = true;
		...
		if (!write_fail) {
			pr_info("Stored soc=%d,currtime_ms=%lu,retry_times=%d\n",
				store_emmc.store_soc, store_emmc.store_currtime_ms/1000, retry_times);
			retry_times = 0;
		} else {
...
int pm8916_bms_consistence_check(void)
{
	...
	if (the_chip->dt.batt_stored_magic_num == BMS_STORE_MAGIC_NUM
			...
			&& (currtime_ms - the_chip->dt.batt_stored_update_time) < 3600) {
