...
/* 1.Disable temp protect. 2.Skip safety timer. 3.Disable safety timer */
		...
		if (vbat_mv > (chip->cfg_max_voltage_mv - VBAT_TOLERANCE_MV)) {
			if ((ibat_ma < 0) && (ibat_ma * (-1) < chip->term_current)) {
				...
				if (eoc_count == CONSECUTIVE_COUNT && !is_batt_full) {
					...
					/* Keep monitor overloading until full_eoc_stop */
					// goto stop_eoc;
				}
			}
		} else {
				...
				int usb_present)
{
	...
	if (!usb_present) {
		...
		/* Clear chag fail bit when cable unplug */
...
int pm8916_set_chg_iusbmax(int val)
{
	...
	/* linear charger iusbmax = ibatmax */
...
int pm8916_get_chg_usb_iusbmax(void)
{
	...
	/* There is no iusbmax, we return ibatmax */
...
{
	/* There is no vchg_loop in pm8916 spec. */
	return 0;
}
...
{
	/* PM8916 is linear charger, not support cc */
	*result = 0;
	return 0;
}
...
#define IDIC_SAMPLE_RATE_SLOW	60000	// 1 minute
#define IDIC_SAMPLE_RATE_FAST	1000	// 1 second
#define IDIC_SAMPLE_FAST_TIMES_MAX	120 // 2 minutes
	...
	if (sample_rate == IDIC_SAMPLE_RATE_SLOW) {
		...
		if (vbat3 && vbat4) {
			if (is_over_lock_voltage || vbat_mv >= IDIC_BATT_LOCK_VOLTAGE) {
				...
				// Re-enable charger to release 4.2V battery protect
				/* Disable charging */
...
#if 0 /* Set charging when usb driver finished cable setection. Not here. */
		if (!usb_present) {
			qpnp_lbc_charger_enable(chip, CURRENT, 0);
...
/*
 *  * Interrupt handler to detect pin 31 of msm8916 ,
 *  * because hardware team has added a protector circuit to this pin,
 *  * which will filter the old error message away if our code does nothing
 *  * LCM has to show error message when pin 31 is pulled low
 *     */
...
static int qpnp_lbc_probe(struct spmi_device *spmi)
{
	...
	if ((chip->revid_data->pmic_subtype == PM8916_V2P0_SUBTYPE) &&
		...
		/*
		 * Per HW Kenny's request, due to PMIC's tolerance,
		 * set 4.325V before V2.0, set 4.35V after V2.0.
		 */
		is_pmic_v20 = true;
 ...
 * we uses late_initcall to init both charger/bms.
 */
